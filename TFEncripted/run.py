...
class PredictionClient():
    ...
    def provide_input(self) -> tf.Tensor:
        with tf.name_scope('loading'):
            prediction_input, expected_result = self._build_data_pipeline().get_next()  # pôr isto como variável global e no ultimo for calcular accuracy/loss
    ...
    def receive_output(self, logits: tf.Tensor) -> tf.Operation:
        with tf.name_scope('post-processing'):
            prediction = tf.argmax(logits, axis=1)  # pôr isto como variável global e no ultimo for calcular accuracy/loss
